...
func State(host string, game_id string) *Game {
	...
	url := fmt.Sprintf("http://%s/api/v1/games/%s/state/", host, safeId)
...
func Pause(host string, game_id string) *Game {
	safeId := url.QueryEscape(game_id)
	url := fmt.Sprintf("http://%s/api/v1/games/%s/pause/", host, safeId)
...
func Resume(host string, game_id string) *Game {
	safeId := url.QueryEscape(game_id)
	url := fmt.Sprintf("http://%s/api/v1/games/%s/resume/", host, safeId)
	req, err := http.NewRequest("POST", url, nil)
	if err != nil {
		log.Fatal("Error in NewRequest: ", err)
		return nil
...
func MarkAsFlag(host string, game_id string, x int, y int) *Game {
	...
	safeId := url.QueryEscape(game_id)
	data, _ := json.Marshal(Arg{X: x, Y: y})
	url := fmt.Sprintf("http://%s/api/v1/games/%s/mark_as_flag/", host, safeId)
	req, err := http.NewRequest("POST", url, bytes.NewBuffer(data))
	if err != nil {
		log.Fatal("Error in NewRequest: ", err)
		return nil
...
func MarkAsQuestion(host string, game_id string, x int, y int) *Game {
	...
	safeId := url.QueryEscape(game_id)
	data, _ := json.Marshal(Arg{X: x, Y: y})
	url := fmt.Sprintf("http://%s/api/v1/games/%s/mark_as_question/", host, safeId)
	req, err := http.NewRequest("POST", url, bytes.NewBuffer(data))
	if err != nil {
		log.Fatal("Error in NewRequest: ", err)
		return nil
...
func Reveal(host string, game_id string, x int, y int) *Game {
	...
	safeId := url.QueryEscape(game_id)
	data, _ := json.Marshal(Arg{X: x, Y: y})
	url := fmt.Sprintf("http://%s/api/v1/games/%s/reveal/", host, safeId)
	req, err := http.NewRequest("POST", url, bytes.NewBuffer(data))
	if err != nil {
		log.Fatal("Error in NewRequest: ", err)
		return nil
